...
class RUEPromptBuilder:
    ...
    def process_project(self, project_name, rq2_data_dir, results_dir, top_k=5):
        inputs_df = pd.read_csv(os.path.join(rq2_data_dir, project_name, 'inputs.csv'))
        meta_df = pd.read_csv(os.path.join(rq2_data_dir, project_name, 'meta_llm.csv'))
        
        emb_suffix = 'bow' if self.embedding_type == 'bow' else self.embedder.model_name
        methods = load_jsonl(os.path.join(results_dir, project_name, f'methods_embeddings_{emb_suffix}.jsonl'))
        ...
        for idx in tqdm(range(len(inputs_df)), desc="Building prompts"):
            ...
            return_type = extract_return_type(focal_method)
            ...
            prompt = build_prompt_with_instruction(focal_method=focal_method, test_prefix=test_prefix, docstring=docstring, context=context)
        ...
        output_dir = os.path.join(results_dir, project_name)
        os.makedirs(output_dir, exist_ok=True)
        save_jsonl(results, os.path.join(output_dir, f'prompts_external_{emb_suffix}.jsonl'))
        return results
...
if __name__ == '__main__':
    ...
    builder.process_project(args.project, args.rq2_data, args.results_dir, args.top_k)
