...
class Dataset(torch.utils.data.Dataset):
    def __init__(self, loaded_data, max_len=512):
        ...
        for inputs, outputs, id, prefix_only in zip(loaded_data['inputs'], loaded_data['outputs'], loaded_data['id'], loaded_data['test_prefix']):
...
class BatchDecodingDataset(torch.utils.data.Dataset):
    def __init__(self, loaded_data, max_len=512):
        self.data = []
        for inputs, outputs, id in zip(loaded_data['inputs'], loaded_data['outputs'], loaded_data['id']):
            if len(inputs) > max_len:
                continue
            ...
            self.data.append({
                'id': id,
                'input_ids': torch.LongTensor(inputs).unsqueeze(0),   # during inference, the input_ids is the prompt (need to be padded in the collator function)
                'labels': torch.LongTensor(outputs).unsqueeze(0),  # labels is used for evaluation
                'attention_mask': torch.ones(1, len(inputs)) # attention mask for padding tokens, 1 for not masked
            })
...
def batch_decoding_collator(data_lst):
    data_batch = {'input_ids': [], 'labels': [], 'attention_mask': [], 'id':[]}
    # inputs (prompts) and outputs (ground-truth) are padded separately for inference
    max_input_len = max([data['input_ids'].size(1) for data in data_lst])
    max_label_len = max([data['labels'].size(1) for data in data_lst])
    for data in data_lst:
        # pad everything to the left for batch inference
        data_batch['input_ids'].append(torch.cat([torch.zeros(1, max_input_len - data['input_ids'].size(1)).long(), data['input_ids']], dim=1))
        data_batch['labels'].append(torch.cat([torch.zeros(1, max_label_len - data['labels'].size(1)).fill_(-100).long(), data['labels']], dim=1))
        data_batch['attention_mask'].append(torch.cat([torch.zeros(1, max_input_len - data['input_ids'].size(1)), data['attention_mask']], dim=1))
        data_batch['id'].append(data['id'])
    data_batch['input_ids'] = torch.cat(data_batch['input_ids'], dim=0)
    data_batch['labels'] = torch.cat(data_batch['labels'], dim=0)
    data_batch['attention_mask'] = torch.cat(data_batch['attention_mask'], dim=0)
    #data_batch['id'] = torch.cat(data_batch['id'], dim=0)
    return data_batch
