...
# Same regex as TOGLL prepare_defects4j_inputs.py
FAIL_CATCH_RE = re.compile(r"fail\(.*\).*}\s*catch", re.MULTILINE | re.DOTALL)


def clean_data(data):
    """Re-indent a stripped test body."""
...
def clean_test_prefix(test_prefix: str) -> str:
    """Strip try/catch/fail blocks and bare assertion lines from test_prefix."""
...
def derive_file_path(test_name: str) -> str:
    """
    Derive a Java file path from test_name.
    e.g. 'com.google.gson.stream.JsonReader_ESTest::test39'
         -> 'com/google/gson/stream/JsonReader_ESTest.java'
    """
    class_part = test_name.split('::')[0]
    return class_part.replace('.', '/') + '.java'


def row_gt_output(row) -> str:
    """GT_output for a single row."""
...
def process(evo_dir: Path, output_dir: Path):
    ...
    # Combine into a single frame (rows are aligned 1-to-1)
    # NO deduplication: keep all 374 rows, same as TOGLL's approach.
    # Each row has a (possibly different) focal_method for the same test_name.
    ...
    print(f"Found {len(projects)} projects: {sorted(projects)}")
    print(f"Total rows (all): {len(combined)}")
    ...
    for project in sorted(projects):
        ...
        for _, row in proj_df.iterrows():
            ...
            all_rows.append({
                ...
                'clean_test_prefix': clean_tp,  # kept for reference, not used in output
                'docstring': doc,
        ...
        # meta_llm.csv: solution_3 metadata — RAW test_prefix so ExceptionClassifier
        # sees the try/catch/fail pattern (crucial for exception classification).
        # TOGLL cleaned it for GPT-2 token budget; LLM agents need full context.
...
def main():
    parser = argparse.ArgumentParser(description='Prepare Defects4J inputs for solution_3')
    parser.add_argument('--evo_dir', type=str,
                        default='togll/RQ5/TOGLL_prediction/evosuite_reaching_tests',
                        help='Path to evosuite_reaching_tests directory')
    parser.add_argument('--output_dir', type=str,
                        default='data/RQ3/input',
                        help='Root output directory (project folders will be created inside)')
