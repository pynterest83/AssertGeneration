...
def load_meta_map(evo_meta_path: Path) -> dict:
    """Build {test_name: (project, bug_num)} from original meta.csv.
    Uses the first (project, bug_num) seen for each test_name.
    """
...
def convert(output_dir: Path, input_dir: Path, evo_meta_path: Path, model_short: str):
    ...
    for idx, row in preds.iterrows():
        ...
        # test_prefix in solution_3 output = clean version (from meta_llm)
        # Fall back to loading from meta_llm if empty
...
def main():
    ...
    parser.add_argument('--output_dir', type=str, default='data/RQ3/output',
                        help='Root dir containing per-project oracle_preds_{model}.csv')
    parser.add_argument('--input_dir', type=str, default='data/RQ3/input',
                        help='Root dir containing per-project infer_input/ (not used yet, reserved)')
    parser.add_argument('--evo_meta', type=str,
                        default='togll/RQ5/TOGLL_prediction/evosuite_reaching_tests/meta.csv',
                        help='Path to original meta.csv from togll RQ5')
    parser.add_argument('--model_name', type=str, required=True,
                        help='Model name as used in run_pipeline.py (e.g. qwen2.5-coder-7b-instruct)')
