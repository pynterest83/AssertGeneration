...
def find_tfp_tests(surefire_bases, project_dir):
    ...
    # Map class names to llm_oracle file paths in project_dir
    # Build index: filename -> [full_paths in llm_oracle]
    llm_files = {}
    for root, dirs, files in os.walk(project_dir):
        parts = Path(root).parts
        if 'llm_oracle' not in parts:
...
def get_test_methods(filepath, normalize=False):
    ...
    """
    method_re = re.compile(r'public\s+void\s+(test0*(\d+)|test(\d+))\s*\(')
...
def main():
    ...
    if args.input_dir:
        ...
        print("=" * 60)
        print("Step 0: Copying input to output directory...")
        print("=" * 60)
        if not args.dry_run:
    ...
    # --- Step 1: Tce ---
    print("=" * 60)
    print("Step 1: Finding Tce (compile error) tests in llm_oracle...")
    print("=" * 60)
    tce_excluded = find_tce_tests(project_dir)
    tce_count = sum(len(v) for v in tce_excluded.values())
    print(f"Found {tce_count} Tce tests across {len(tce_excluded)} files")
    for f, names in sorted(tce_excluded.items()):
        print(f"  {os.path.relpath(f, project_dir)}: "
              f"{sorted(names, key=get_test_num)}")

    # --- Step 2: Tfp ---
    print()
    print("=" * 60)
    print("Step 2: Finding Tfp (false positive) tests from surefire...")
    print("=" * 60)
    tfp_excluded = find_tfp_tests(surefire_bases, project_dir)
    tfp_count = sum(len(v) for v in tfp_excluded.values())
    print(f"Found {tfp_count} Tfp tests across {len(tfp_excluded)} files")
    for f, names in sorted(tfp_excluded.items()):
        print(f"  {os.path.relpath(f, project_dir)}: "
              f"{sorted(names, key=get_test_num)}")

    # --- Step 3: Exception tests (bidirectional mismatch) ---
    print()
    print("=" * 60)
    print("Step 3: Finding method mismatches between src and llm_oracle...")
    print("=" * 60)
    ...
    print(f"Found {exc_count} exception tests (in src, not llm) "
          f"across {len(exc_excluded)} files")
    print(f"Found {llm_extra_count} extra llm tests (in llm, not src) "
          f"across {len(llm_extra_excluded)} files")
    for f, names in sorted(exc_excluded.items()):
        print(f"  [src-only] {os.path.relpath(f, project_dir)}: "
              f"{sorted(names, key=get_test_num)}")
    for f, names in sorted(llm_extra_excluded.items()):
        print(f"  [llm-only] {os.path.relpath(f, project_dir)}: "
              f"{sorted(names, key=get_test_num)}")
    ...
    print(f"\nTotal: {total_tce_tfp} Tce+Tfp + {exc_count} exception "
          f"+ {llm_extra_count} llm-extra = {total_excluded} to exclude")

    if args.dry_run:
        print("\n[DRY RUN] No files modified.")
        print()
        print("=" * 60)
        print("Pre-check: Current active test counts across suites...")
        print("=" * 60)
        ...
        if ok:
            print("ALL FILES MATCH: per-file active counts are equal across suites.")
        else:
            print(f"\n{len(mismatches)} file(s) have mismatched active test counts ")
            print("(these will be equalized after actual run):")
            for mm in mismatches:
                print(f"  {mm['file']}:")
                for suite, cnt in sorted(mm['counts'].items()):
                    print(f"    {suite}: {cnt} active @Test")
        return

    # --- Step 4: Merge all exclusions per-file, then comment @Test ---
    # Build unified per-file exclusion dicts so each file is processed ONCE.
    # This prevents duplicate test_nothing() and double-commenting @Test.
    llm_exclusions = defaultdict(set)   # llm_oracle file -> methods
    src_exclusions = defaultdict(set)   # src file -> methods
    no_exclusions = defaultdict(set)    # no_oracle file -> methods

    # 4a. Tce + Tfp → affects all 3 suites
    for llm_file, methods in all_excluded_llm.items():
        llm_exclusions[llm_file].update(methods)
        src_file, no_oracle_file = get_corresponding_paths(
            llm_file, project_dir)
        if src_file:
            src_exclusions[src_file].update(methods)
        if no_oracle_file:
            no_exclusions[no_oracle_file].update(methods)

    # 4b. Exception tests (src→not in llm) → affects ALL 3 suites
    ...
    # 4c. Extra llm tests (llm→not in src) → affects only llm_oracle
    for llm_file, methods in llm_extra_excluded.items():
        llm_exclusions[llm_file].update(methods)

    print()
    print("=" * 60)
    print("Step 4: Commenting @Test in all suites...")
    print("=" * 60)

    total_commented = {'llm_oracle': 0, 'src': 0, 'no_oracle': 0}
    all_processed_files = set()
    sort_key = get_test_num

    for llm_file, methods in sorted(llm_exclusions.items()):
        print(f"\n  [llm_oracle] {os.path.relpath(llm_file, project_dir)}")
        print(f"    Excluding: {sorted(methods, key=sort_key)}")
        c = comment_tests_in_file(llm_file, methods)
        total_commented['llm_oracle'] += c
        all_processed_files.add(llm_file)
        print(f"    Commented: {c} @Test")

    for src_file, methods in sorted(src_exclusions.items()):
        print(f"\n  [src] {os.path.relpath(src_file, project_dir)}")
        print(f"    Excluding: {sorted(methods, key=sort_key)}")
        c = comment_tests_in_file(src_file, methods)
        total_commented['src'] += c
        all_processed_files.add(src_file)
        print(f"    Commented: {c} @Test")

    for no_file, methods in sorted(no_exclusions.items()):
        print(f"\n  [no_oracle] {os.path.relpath(no_file, project_dir)}")
        print(f"    Excluding: {sorted(methods, key=sort_key)}")
        c = comment_tests_in_file(no_file, methods)
        total_commented['no_oracle'] += c
        all_processed_files.add(no_file)
        print(f"    Commented: {c} @Test")

    # --- Step 5: Disable separateClassLoader everywhere ---
    print()
    print("=" * 60)
    print("Step 5: Disabling separateClassLoader in remaining files...")
    print("=" * 60)
    cl_fixed = disable_classloader_all(project_dir, all_processed_files)
    print(f"Fixed separateClassLoader in {cl_fixed} additional files")

    # --- Step 6: Verify test counts ---
    print()
    print("=" * 60)
    print("Step 6: Verifying test counts match across suites...")
    print("=" * 60)
    ok, mismatches, suite_totals = verify_test_counts(project_dir)
    for s in ('src', 'llm_oracle', 'no_oracle'):
        print(f"  {s}: {suite_totals.get(s, 0)} active @Test remaining")
    if ok:
        print("PASSED: per-file active counts are equal across all 3 suites.")
    else:
        print(f"FAILED: {len(mismatches)} file(s) have mismatched active test counts:")
        for mm in mismatches:
            print(f"  {mm['file']}:")
            for suite, cnt in sorted(mm['counts'].items()):
                print(f"    {suite}: {cnt} active @Test")
    ...
    results = {
        ...
        'tce_tfp_files': {
            os.path.relpath(f, project_dir): sorted(list(names), key=sort_fn)
    ...
    print("Summary")
    print("=" * 60)
    print(f"Tce (compile error tests):     {tce_count}")
    print(f"Tfp (false positive tests):    {tfp_count}")
    print(f"Exception tests:               {exc_count}")
    print(f"Total excluded:                {total_excluded}")
    print(f"Active remaining (src):        {suite_totals.get('src', 0)}")
    print(f"Active remaining (llm_oracle): {suite_totals.get('llm_oracle', 0)}")
    print(f"Active remaining (no_oracle):  {suite_totals.get('no_oracle', 0)}")
    print(f"separateClassLoader disabled:  {cl_fixed} additional files")
    print(f"Verification:                  {'PASSED' if ok else f'FAILED ({len(mismatches)} mismatches)'}")
